...
// AITUNNEL_KEY = ваш ключ от AITunnel
// YOOMONEY_WALLET = номер кошелька ЮMoney (4100...)
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const path = url.pathname.slice(1); // убираем первый слэш: "/ai" → "ai"

      console.log('Request path:', path);

      // === AI CHAT ===
      if (path === 'ai') {
        if (!apiKey) return new Response(JSON.stringify({ error: 'AITUNNEL_KEY not set' }), {
      ...
      if (path === 'image') {
        ...
        return new Response(JSON.stringify({ image_url: data.data?.[0]?.url || null }), {
      ...
      if (path === 'video') {
        ...
        const data = await response.json();
        return new Response(JSON.stringify({ video_url: data.data?.[0]?.url || null }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
      }
      ...
      if (path === 'pay') {
        ...
        if (!wallet) return new Response(JSON.stringify({ error: 'YOOMONEY_WALLET not set' }), {
          status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
        ...
        const params = new URLSearchParams({
          receiver: wallet, quickpay_form: 'shop',
          targets: `Scoffe2 — тариф ${tariffName}`, paymentType: 'AC',
      ...
      return new Response(JSON.stringify({ 
        error: 'Not found', 
        path: path,
        available: ['ai', 'image', 'video', 'rerank', 'pay']
      }), { 
        status: 404, 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      });
